...
6. **First-introduced version and fix status.** No product changelog or source history exists locally; when this behavior was introduced, and whether any build changes it, is UNKNOWN. The server bundle auto-updates independently, so behavior may change at any time.
